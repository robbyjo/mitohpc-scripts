...
readonly SCHEDULER="${MITO_PIPELINE_SCHEDULER:?internal error: scheduler backend is not set}"
readonly PROGRAM="${MITO_PIPELINE_PROGRAM:-mito_pipeline.sh}"
...
usage() {
    printf 'Usage:\n  %s INPUT_DIR OUTPUT_DIR [options]\n' "$PROGRAM"
    cat <<'EOF'
...
Scheduler resource options:
  --cpus N                  CPUs/slots per task (default: 4)
  --memory SIZE             Total memory per task, e.g. 16G (default: 16G)
  --tool-memory SIZE        Per-process/tool memory limit (default: 3G)
  --time D-HH:MM:SS         Wall time per array task (default: 2-00:00:00)
  --max-parallel N          Maximum simultaneous array tasks (default: 20)
  --max-array-size N        Maximum tasks in one array (default: 1000)
  --max-user-jobs N         Running-plus-pending task limit (default: 1000)
  --job-headroom N          Task slots reserved for other work (default: 20)
  --mail-user ADDRESS       Scheduler notification address
  --mail-type TYPES         BEGIN,END,FAIL,ALL,NONE (default: FAIL)
  --prologue FILE           Shell file sourced in every compute job
...
Open Grid Scheduler-only options:
  --queue NAME              Grid Engine queue
  --project NAME            Grid Engine project (-P)
  --parallel-env NAME       Parallel environment (-pe; default: smp)
  --memory-resource NAME    Per-slot memory complex (default: h_vmem)
  --time-resource NAME      Wall-time complex (default: h_rt)
  --qsub-resource SPEC      Additional Grid Engine -l request; repeatable

Control options:
  --require-indexes         Fail instead of scheduling creation of missing indexes
  --dry-run                 Validate and prepare a run without submitting
  --version                 Print version
  -h, --help                Show this help

Use mito_pipeline.sh for SLURM, mito_pipeline_qsub.sh for qsub dialect
detection, or mito_pipeline_auto.sh for scheduler auto-detection. Rerunning the
same command is safe: samples with a success marker are skipped.
EOF
}
...
while (($#)); do
    case "$1" in
        --mitohpc-dir|--reference-profile|--reference-fasta|--mt-contig|--caller|--iterations|--max-mt-reads|--min-depth|--cpus|--memory|--tool-memory|--time|--max-parallel|--max-array-size|--max-user-jobs|--job-headroom|--partition|--account|--qos|--queue|--project|--parallel-env|--memory-resource|--time-resource|--qsub-resource|--mail-user|--mail-type|--prologue)
            ...
            case "$option" in
                ...
                --parallel-env) parallel_env=$value ;;
                --memory-resource) memory_resource=$value ;;
                --time-resource) time_resource=$value ;;
                --qsub-resource) qsub_resources+=("$value") ;;
...
[[ "$SCHEDULER" =~ ^(slurm|ogs)$ ]] || die "unsupported scheduler backend: $SCHEDULER"
if [[ "$SCHEDULER" == slurm ]]; then
    [[ -z "$queue$project${qsub_resources[*]}" ]] || die 'Grid Engine options require mito_pipeline_qsub.sh'
else
    [[ -z "$partition$account$qos" ]] || die 'use --queue/--project, not SLURM --partition/--account/--qos, with qsub'
fi
[[ "$parallel_env" =~ ^[A-Za-z0-9_.-]+$ ]] || die 'parallel-env contains unsafe characters'
[[ "$memory_resource" =~ ^[A-Za-z][A-Za-z0-9_.-]*$ ]] || die 'memory-resource contains unsafe characters'
[[ "$time_resource" =~ ^[A-Za-z][A-Za-z0-9_.-]*$ ]] || die 'time-resource contains unsafe characters'
[[ -z "$queue" || ("$queue" != -* && "$queue" != *[[:space:]]*) ]] || die 'queue contains unsafe characters'
[[ -z "$project" || "$project" =~ ^[A-Za-z0-9_.-]+$ ]] || die 'project contains unsafe characters'
for qsub_resource in "${qsub_resources[@]}"; do
    [[ -n "$qsub_resource" && "$qsub_resource" != -* && "$qsub_resource" != *$'\t'* && "$qsub_resource" != *$'\n'* ]] ||
        die 'qsub-resource must be a non-empty Grid Engine resource request'
done
...
if ((!dry_run)); then
    if [[ "$SCHEDULER" == slurm ]]; then
        command -v sbatch >/dev/null 2>&1 || die 'sbatch is not available; use --dry-run off-cluster'
        command -v squeue >/dev/null 2>&1 || die 'squeue is not available'
    else
        command -v qsub >/dev/null 2>&1 || die 'qsub is not available; use --dry-run off-cluster'
        command -v qstat >/dev/null 2>&1 || die 'qstat is not available'
        if command -v qconf >/dev/null 2>&1; then
            pe_list=$(qconf -spl 2>&1) || die "could not list Grid Engine parallel environments: $pe_list"
            grep -Fx -- "$parallel_env" <<< "$pe_list" >/dev/null ||
                die "Grid Engine parallel environment '$parallel_env' does not exist; available values: $(tr '\n' ' ' <<< "$pe_list")"
            complex_list=$(qconf -sc 2>&1) || die "could not list Grid Engine resource complexes: $complex_list"
            for resource_name in "$memory_resource" "$time_resource"; do
                awk -v name="$resource_name" '$1 == name {found=1} END {exit !found}' <<< "$complex_list" ||
                    die "Grid Engine resource complex '$resource_name' does not exist; override its launcher option"
            done
        else
            log 'qconf is unavailable; skipping Grid Engine parallel-environment and complex preflight'
        fi
    fi
fi
...
if [[ "$SCHEDULER" == ogs ]]; then
    for helper in bundle_array.sh summary.sh; do
        [[ -r "$SCRIPT_DIR/ogs/$helper" ]] || die "required Grid Engine helper is not readable: $SCRIPT_DIR/ogs/$helper"
    done
fi
...
if ((!dry_run)); then
    ...
    if [[ "$SCHEDULER" == slurm ]]; then
        ...
        if ! raw_queue_snapshot=$(qstat -u "$queue_user" -g d 2>&1); then
            die "could not query active Grid Engine jobs: $raw_queue_snapshot"
        fi
        queue_snapshot=$(awk '$1 ~ /^[0-9]+$/ {print $1 "|" $5}' <<< "$raw_queue_snapshot")
    fi
    while IFS= read -r previous_metadata; do
        previous_scheduler=$(awk -F '\t' '$1 == "scheduler" {print $2; exit}' "$previous_metadata")
        [[ -n "$previous_scheduler" ]] || previous_scheduler=slurm
        [[ "$previous_scheduler" == "$SCHEDULER" ]] || continue
...
if ((bundle_size > 1)); then
    ...
    log "--time applies to the whole bundle; choose a suitable queue/partition and wall time"
fi
...
ogs_walltime() {
    local value=$1 days=0 clock hours minutes seconds
    if [[ "$value" == *-* ]]; then
        days=${value%%-*}
        clock=${value#*-}
    else
    ...
    seconds=${seconds:-0}
    printf '%02d:%02d:%02d\n' "$((10#$days * 24 + 10#$hours))" "$((10#$minutes))" "$((10#$seconds))"
}

ogs_mail_events() {
    local input=$1 token result=''
    ...
    for token in "${tokens[@]}"; do
        case "$token" in
            ...
            ALL) result=beas ;;
            NONE) result=n ;;
            *) die "unsupported Grid Engine mail type: $token (use BEGIN,END,FAIL,ALL,NONE)" ;;
        esac
    done
    printf '%s\n' "$result"
}
...
build_ogs_common() {
    local -n target=$1
    local requested_cpus=$2 requested_memory=$3 requested_time=$4
    local total_mib per_slot_mib wall resource
    total_mib=$(memory_to_mib "$requested_memory")
    per_slot_mib=$(((total_mib + requested_cpus - 1) / requested_cpus))
    wall=$(ogs_walltime "$requested_time")
    target=(-terse -V -cwd -S /bin/bash -pe "$parallel_env" "$requested_cpus"
        -l "$memory_resource=${per_slot_mib}M" -l "$time_resource=$wall")
    [[ -z "$queue" ]] || target+=(-q "$queue")
    [[ -z "$project" ]] || target+=(-P "$project")
    for resource in "${qsub_resources[@]}"; do
        target+=(-l "$resource")
    done
    if [[ -n "$mail_user" ]]; then
        target+=(-M "$mail_user" -m "$(ogs_mail_events "$mail_type")")
    fi
}
...
run_qsub() {
    local output
    if ! output=$(qsub "$@" 2>&1); then
        printf 'ERROR: qsub failed: %s\n' "$output" >&2
        printf 'ERROR: verify --parallel-env, Grid Engine resource names, queue policy, and user job limits.\n' >&2
        return 1
    fi
    printf '%s\n' "$output"
}
...
if [[ "$SCHEDULER" == slurm ]]; then
    ...
    build_ogs_common common_qsub "$cpus" "$memory" "$walltime"
fi

submit_chunked_arrays() {
    local -n result_jobs=$1
    local total=$2 stage=$3 job_name=$4 log_prefix=$5 metadata_key=$6 base_dependency=$7
    local offset items count array_spec dependency job_output job_id previous_job='' parallel ogs_dependencies
    ...
    for ((offset = 0; offset < total; offset += max_items_per_array)); do
        ...
        if [[ "$SCHEDULER" == slurm ]]; then
            ...
            job_output=$(run_sbatch "${common_sbatch[@]}" "${dependency_arg[@]}" \
                --job-name="$job_name" \
                --array="$array_spec" \
            ...
            if [[ -n "$dependency" ]]; then
                ogs_dependencies=$(dependency_job_ids "$dependency")
                dependency_arg=(-hold_jid "$ogs_dependencies")
            fi
            job_output=$(run_qsub "${common_qsub[@]}" "${dependency_arg[@]}" \
                -N "$job_name" -t "1-$count" -tc "$parallel" \
                -o "$logs_dir" -e "$logs_dir" \
                "$SCRIPT_DIR/ogs/bundle_array.sh" "$SCRIPT_DIR/slurm" "$stage" "$config" \
                "$offset" "$bundle_size" "$total" "$logs_dir" "$log_prefix")
            job_id=$(parse_qsub_job_id "$job_output") || die "could not parse $job_name qsub job ID: $job_output"
            log "submitted $job_name array job $job_id: -t 1-$count -tc $parallel, manifest offset $offset, bundle size $bundle_size"
        fi
        result_jobs+=("$job_id")
...
if ((!extract_only)); then
    ...
    if [[ "$SCHEDULER" == slurm ]]; then
        ...
        summary_qsub=()
        build_ogs_common summary_qsub 1 8G 04:00:00
        ogs_summary_dependencies=$(dependency_job_ids "$summary_dependency")
        summary_output=$(run_qsub "${summary_qsub[@]}" \
            -hold_jid "$ogs_summary_dependencies" \
            -N mitohpc_summary -o "$logs_dir" -e "$logs_dir" \
            "$SCRIPT_DIR/ogs/summary.sh" "$SCRIPT_DIR/slurm" "$config" "$logs_dir" summary)
        summary_job=$(parse_qsub_job_id "$summary_output") ||
            die "could not parse summary qsub job ID: $summary_output"
    fi
